...
#2 Total sales 
 ...
 select round(sum(sales),2) as total_sales from grocery;
 
select * from Total_Sale;
 
#3 Avg sales between year 2015 and 2020
create view Avg_Sales as
select round(avg(sales),2)as sales from grocery
...
#6 Sales summary according to fields
...
#7 Total Sales By Items

create view sales_by_items as
SELECT Item_Type, round(SUM(Sales),2) AS Total_Sales from grocery
GROUP BY Item_Type
ORDER BY Total_Sales DESC;

select * from sales_by_items;
...
#9 creating a short metrics for tables
...
#10 Sales and Avg sales by Outlet Location
select Outlet_Location_Type, round(sum(Sales),2)as sale, 
round(avg(Sales),2)as avg_sale from grocery
